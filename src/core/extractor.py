...
class RuleBasedExtractor(SignalExtractor):
    ...
    - Novelty: Entropy of actions + frustration scaling
    ...
    def extract(self, observation: Observation) -> Signals:
        ...
        # 3. Apply Frustration Dominance (Novelty Suppression)
        # N-2: Novelty scaled by (1 - frustration^2)
        # Frustration is roughly approximated by difficulty for extraction purposes
    ...
    def _compute_novelty(self, obs: Observation, state_delta: float) -> float:
        ...
        # Novelty Debt Recovery (Spec Fix)
    ...
    def _compute_difficulty(self, obs: Observation, state_delta: float) -> float:
        ...
        # Add stagnation penalty (Spec: Frustration spike after N steps)
